...
;(() => {
  ...
  // ---- addon config (the 6s addon differs only in this block) --------------
  const ADDON_ID = 'demucs-v4'
  const MENU_LABEL = '🎚 Demucs v4'
  ...
  function openStemsWindow(track) {
    ...
    function setStatusReady() {
      const modeNote = ftMode() ? 'fine-tuned (best quality, slower)' : 'fast (recommended)'
      $('status').textContent = `ready — model: ${modeNote}. Adjust ⚙ settings if you like, then press Start.`
    }
